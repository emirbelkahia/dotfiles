...
- `~/Notes/Calls/*_prompt_debug.log` — 49 debug log files accumulated from past `tc` runs (April 2025 → March 2026). Generated by `exec > >(tee -a)` on every invocation, served no ongoing purpose.
